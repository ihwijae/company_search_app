...
function AgreementsPage() {
  ...
  const [typeKey, setTypeKey] = useState('eung');
              ...
              <div className="search-filter-section">
                <div className="filter-grid">
                  ...
                  <div className="filter-item" style={{ gridColumn: 'span 2' }}>
                    <label>공고명</label>
                    <input type="text" className="filter-input" value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="예: 가칭)호명중학교 교사 신축 소방공사 계약" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="panel" style={{ gridColumn: '1 / -1' }}>
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h4 style={{ margin: 0 }}>구성사 (최대 4)</h4>
                  <button className="no-drag" onClick={addMember} disabled={members.length >= 4}>구성사 추가</button>
                </div>
                {members.map((m, idx) => (
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {inlineIdx === idx ? (
                        <>
                          ...
                          <button className="no-drag" onClick={saveInlineEdit} disabled={inlineTotal !== 100}>지분 저장</button>
